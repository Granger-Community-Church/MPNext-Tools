...
12. **Never send UTC timestamps to Ministry Platform** - MP stores datetime values without timezone awareness. `new Date().toISOString()` produces UTC (e.g., `2026-04-16T03:02:00Z`), which MP interprets as local time, causing offsets (4+ hours in ET). Always format dates as local time without a timezone suffix (e.g., `2026-04-15T23:02:00`). Generate timestamps on the **client** side using local time formatting, not on the server with `.toISOString()`.
