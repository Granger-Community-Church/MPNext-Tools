...
export const FormFieldsSchema = z.object({
  ...
  Attribute_ID: z.number().int().nullable(),
  Days_Attribute_Is_Valid: z.number().int(),
  Feedback_Type_ID: z.number().int().nullable(),
  Milestone_ID: z.number().int().nullable(),
  Program_ID: z.number().int().nullable(),
  Milestone_Value: z.string().max(4000).nullable(),
  PlusOne_Field: z.number().int().nullable(),
  Is_Hidden: z.boolean(),
